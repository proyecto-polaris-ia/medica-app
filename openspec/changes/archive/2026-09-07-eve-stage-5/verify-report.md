...
- **Second deploy fix**: after the gate, `eve build` failed with `Expected the channel export "default" ... to match the public eve shape`. Eve resolves an authored channel module by its default export, but `chatSdkChannel` returns `{ bot, channel, send }`; the earlier sample exported only named members. The module now re-exports `channel` as `default` (verified by a full local `eve build` succeeding under Node 24). The original Issue #35 code sample omitted this default export.
